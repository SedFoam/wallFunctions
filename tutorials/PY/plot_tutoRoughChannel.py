...
for case in caseList:
    ...
    axU.plot(Ux, Zmesh / Hwater, lw=lw, ls=ls, color=color, label=label)

    # k, specific turbulent kinetic enery
    kField = rdf.readscalar(pathCase, time, "k")
    axK.plot(kField, Zmesh / Hwater, lw=lw, ls=ls, color=color)
    # omega
    omField = rdf.readscalar(pathCase, time, "omega")
    axOm.plot(omField, Zmesh / Hwater, lw=lw, ls=ls, color=color)
    zmAx.plot(omField, Zmesh / Hwater, lw=lw, ls=ls, color=color)

    # turbulent eddy viscosity
    nutField = rdf.readscalar(pathCase, time, "nut")
    axNut.plot(nutField, Zmesh / Hwater, lw=lw, ls=ls, color=color)
    ...
    for i, t in enumerate(timeList):
        ...
        tauWall = rdf.readvector(
            pathCase, t, "wallShearStress", boundary="roughWall", verbose=False
        )[0, 0]
        ustarArr[i] = np.sqrt(np.abs(tauWall))
        yPlusArr[i] = rdf.readscalar(
            pathCase, t, "yPlus", boundary="roughWall", verbose=False
        )[0]
    ksPlusArr = ksExp * ustarArr / nuF
    axUstar.plot(timeArr, ustarArr, lw=lw, ls=ls, color=color, label=label)
    axYplus.plot(timeArr, yPlusArr, lw=lw, ls=ls, color=color)
    axKsPlus.plot(timeArr, ksPlusArr, lw=lw, ls=ls, color=color)

axU.scatter(
    uxDat * ustarExp,
    zuksDat * ksExp / Hwater,
    color="firebrick",
    marker="x",
    label="Fuhrman (2010)",
)
